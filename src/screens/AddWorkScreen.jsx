...
const AddWorkScreen = ({ navigation }) => {
  ...
  const submit = async () => {
    ...
    if (!isPublic && (!price || !currencyId)) { setError('Indiquez le prix et la devise de consultation.'); return; }
    setSubmitting(true); setError('');
    const data = new FormData();
    data.append('work_title', title.trim()); data.append('work_content', description.trim()); data.append('work_url', url.trim());
    data.append('author', author.trim()); data.append('editor', editor.trim()); data.append('is_public', isPublic ? '1' : '0');
    data.append('consultation_price', isPublic ? '' : price); data.append('currency_id', isPublic ? '' : String(currencyId));
    ...
    try {
      const response = await fetch(`${API.boongo_url}/work`, { method: 'POST', headers: { 'X-localization': 'fr', Authorization: `Bearer ${userInfo.api_token}` }, body: data });
      const responseData = await response.json().catch(() => ({}));
      if (!response.ok || responseData.success === false) throw new Error(responseData.message || 'La publication a échoué.');
  ...
  return <SafeAreaView style={[styles.screen, { backgroundColor: COLORS.light }]} edges={['top']}><HeaderComponent title={t('work.publish_new')} /><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    ...
    <Text style={[styles.sectionTitle, { color: COLORS.black }]}>{t('work.is_public.title')}</Text><View style={styles.visibility}>{[true, false].map(value => <TouchableOpacity key={String(value)} onPress={() => setIsPublic(value)} style={[styles.visibilityOption, { backgroundColor: isPublic === value ? COLORS.light_primary : COLORS.white, borderColor: isPublic === value ? COLORS.primary : COLORS.light_secondary }]}><Icon name={value ? 'earth' : 'lock-outline'} size={18} color={isPublic === value ? COLORS.primary : COLORS.dark} /><Text style={[styles.visibilityText, { color: COLORS.black }]}>{value ? t('yes') : t('no')}</Text></TouchableOpacity>)}</View>
    {!isPublic ? <View style={[styles.card, { backgroundColor: COLORS.white, borderColor: COLORS.light_secondary }]}><TextInput value={price} onChangeText={setPrice} keyboardType="decimal-pad" placeholder={t('work.is_public.consult_price')} placeholderTextColor={COLORS.dark} style={[styles.input, { color: COLORS.black, borderColor: COLORS.light_secondary }]} /><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chips}>{currencies.map(item => chip(item, currencyId === item.id, () => setCurrencyId(item.id), item.currency_acronym || item.currency_name))}</ScrollView></View> : null}
