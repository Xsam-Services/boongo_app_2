...
const EstablishmentScreen = () => {
  ...
  const fetchOrganizations = async (pageToFetch = 1) => {
    ...
    const url = `${API.boongo_url}/organization/find_all_by_type/34`;
    const mHeaders = {
      'X-localization': 'fr',
      'X-user-id': userInfo.id,
      'Authorization': `Bearer ${userInfo.api_token}`
  ...
  const OrganizationItem = ({ item }) => {
    ...
    return (
      <Pressable onPress={() => { navigation.navigate('OrganizationData', { organization_id: item.id, type: 'establishment' }); }} style={{ flexDirection: 'row', alignItems: 'center', padding: PADDING.p03, backgroundColor: COLORS.white }}>
  ...
  return (
    <>
      ...
      {/* Floating button */}
      <Pressable style={[homeStyles.floatingButton, { bottom: 30, backgroundColor: COLORS.success }]} onPress={() => { navigation.navigate('AddEstablishment') }}>
        ...
        <SafeAreaView contentContainerStyle={{ flexGrow: 1 }}>
          <View style={[homeStyles.cardEmpty, { height: Dimensions.get('window').height, marginLeft: 0, paddingHorizontal: 2 }]}>
            ...
            <Animated.FlatList
              ...
              ListEmptyComponent={<EmptyListComponent iconName='bank-outline' title={t('empty_list.title')} description={t('empty_list.description_establishments')} />}
              ListFooterComponent={() =>
                isLoading ? (
