...
const JournalScreen = () => {
  ...
  const handleBadgePress = useCallback((id) => {
    setIdCat(id);
    mags.splice(0, mags.length);

    // Reload data
    getMags2(id);
    console.log('handleReload => Works count: ' + mags.length + ', Selected category: ' + idCat);
  }, []);
  ...
  const getMags = () => {
    ...
    let qs = require('qs');
    const url = `${API.boongo_url}/work/filter_by_categories_type_status/fr/Article/Pertinente`;
    ...
    axios.post(url, qs.stringify(mParams), mHeaders).then(res => {
      ...
      console.log(new Date() + ' : getMags => Works count: ' + magsData.length + ', Selected category: ' + idCat);
  ...
  const getMags2 = (id) => {
    ...
    axios.post(url, qs.stringify(mParams), mHeaders).then(res => {
      ...
      console.log(new Date() + ' : getMags2 => Works count: ' + magsData.length + ', Selected category: ' + id);
  ...
  //   return (
  //     <View>
  //       <TouchableOpacity activeOpacity={0.9} onPress={getMags} style={[homeStyles.authButton, { marginBottom: 30, paddingVertical: PADDING.vertical, borderRadius: 30 }]}>
            ...
            ListEmptyComponent={() => {
              return (
                <>
                  ...
                  <Text style={[homeStyles.cardEmptyText, { marginBottom: 25 }]}>{t('empty_list.description_mags')}</Text>
                </>
              )
            }}
            renderItem={({ item }) => {
              return (<MagItem item={item} />);
            }}
            // ListFooterComponent={mags.length > 0 ? currentPage === lastPage ? null : renderLoadMoreButton : null}
